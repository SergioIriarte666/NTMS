...
router.get('/', requireAuth, (req: AuthedRequest, res: Response) => {
  requirePermission(canRead, req, res, () => {
    void (async () => {
      ...
      if (error) {
        sendSupabaseError(res, 500, error, 'No se pudieron cargar los proveedores')
        return
      }
...
router.post('/', requireAuth, (req: AuthedRequest, res: Response) => {
  requirePermission(canWriteCore, req, res, () => {
    ...
    void (async () => {
      ...
      if (error || !data) {
        sendSupabaseError(res, 400, error, 'No se pudo crear el proveedor')
        return
      }
...
router.patch('/:id', requireAuth, (req: AuthedRequest, res: Response) => {
  requirePermission(canWriteCore, req, res, () => {
    ...
    void (async () => {
      ...
      if (error) {
        sendSupabaseError(res, 400, error, 'No se pudo actualizar el proveedor')
        return
      }
      if (!data) {
...
router.delete('/:id', requireAuth, (req: AuthedRequest, res: Response) => {
  requirePermission(canWriteCore, req, res, () => {
    ...
    void (async () => {
      ...
      if (error) {
        sendSupabaseError(res, 400, error, 'No se pudo eliminar el proveedor')
        return
      }
